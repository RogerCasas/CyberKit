"""
CyberKit — Reflected XSS Tester Page (detection only)
"""
...
from app.modules.xss_tester import XSSResult, scan as xss_scan
...
class XssTesterPage(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            hdr, text="🧨  Reflected XSS Tester",
            font=ctk.CTkFont(family="Segoe UI", size=26, weight="bold"),
            text_color=TEXT_PRIMARY, anchor="w",
        ).grid(row=0, column=0, sticky="w")
        ctk.CTkLabel(
            hdr,
            text="Detect reflected cross-site scripting  •  Detection only, no browser execution",
    ...
    def _build_controls(self):
        ...
        self._url_entry = ctk.CTkEntry(
            ctrl,
            placeholder_text="http://localhost/search?q=test",
    ...
    def _build_params(self):
        ...
        ctk.CTkLabel(
            card,
            text="Payloads: <script>, \"><img onerror>, <svg onload>, attribute breakout — "
                 "each carries a unique marker; raw (unescaped) reflection is flagged vulnerable.",
            font=ctk.CTkFont(family="Segoe UI", size=11),
    ...
    def _build_table(self):
        ...
        ctk.CTkLabel(
            wrapper, text="Reflection Results",
            font=ctk.CTkFont(family="Segoe UI", size=12, weight="bold"),
            text_color=TEXT_MUTED, anchor="w",
        ).grid(row=0, column=0, sticky="w", padx=16, pady=(12, 8))

        style = ttk.Style()
        style.configure("XSS.Treeview",
        ...
        vsb = ttk.Scrollbar(tree_frame, orient="vertical",
                            style="XSS.Vertical.TScrollbar")
        vsb.grid(row=0, column=1, sticky="ns")

        self._tree = ttk.Treeview(
            tree_frame,
            columns=("param", "payload", "reflected", "encoded", "context", "verdict"),
            show="headings", style="XSS.Treeview",
        ...
        for col, head, w, anc, stretch, mw in [
            ...
            ("payload",   "Payload",   240, "w",      True,  120),
            ("reflected", "Reflected", 80,  "center", False, 70),
            ("encoded",   "Encoded",   80,  "center", False, 70),
            ("context",   "Context",   90,  "center", False, 70),
    ...
    def _insert_row(self, r: XSSResult):
        tag = "vulnerable" if r.is_vulnerable else "clean"
        alt = "row_even" if self._row_count % 2 == 0 else "row_odd"
        verdict = "VULNERABLE" if r.is_vulnerable else ("Reflected" if r.reflected else "Clean")
        self._tree.insert(
            "", "end",
            values=(
                r.parameter, r.payload,
                "Yes" if r.reflected else "No",
                "Yes" if r.encoded else "No",
                r.context, verdict,
            ),
            tags=(tag, alt),
        )
    ...
    def _finish_scan(self, stopped: bool):
        ...
        self._status_lbl.configure(
            text=f"Done{suffix}  —  {vuln} vulnerable finding(s) across {self._row_count} parameter(s)",
